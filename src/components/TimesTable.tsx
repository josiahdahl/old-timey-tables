...
export function TimesTable({ showResults, showSettings }: TimesTableProps) {
  const {
    rows,
    cols,
    cells,
  ...
  return rows.length === 0 ? null : (
    <section className="relative">
      <form
        ...
        className={`transition duration-300 ${state === TimesTableState.IDLE ? "blur filter" : ""}`}
      >
        <table ref={tableRef}>
          <thead>
            <tr>
        ...
        <div
          className={`flex justify-center py-4 ${
            state === TimesTableState.IDLE ? "invisible" : ""
          }`}
        >
      ...
      {state === TimesTableState.IDLE && (
        <section className="absolute top-0 left-0 right-0 bottom-0 flex flex-col justify-center items-center gap-y-5">
          <button
            className="rounded border-2 border-blue-900 bg-blue-900 text-white font-bold px-2 py-1"
            onClick={start}
          >
            Start!
          </button>
